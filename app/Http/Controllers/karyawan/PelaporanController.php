...
class PelaporanController extends Controller
{
    public function index()
    {
        ...
        ->where('user_id', Auth::id()) // Hanya pelaporan milik user login
        ->latest()
        ->paginate(10);
        $historiPelaporans = Histori::with(['pelaporan.user'])
        ->whereHas('pelaporan', function ($query) {
            $query->where('user_id', Auth::id()); // Hanya histori milik user login
        })
        ->latest()
        ->paginate(10);
    
        $kategoris = Kategori::all();
        return view('karyawan.pelaporanTable', compact('pelaporans','kategoris', 'historiPelaporans'));
    }

    public function create()
    {
        $users = User::where('jabatan', 'karyawan')->get();
        return view('karyawan.pelaporanAdd',compact('users'));
    }
...
$kodeUnik = 'SPKR' . str_pad($jumlahPelaporan, 3, '0', STR_PAD_LEFT);
    ...
    public function show(string $id)
    {
        ...
        return view('karyawan.pelaporanShow', compact('pelaporan'));
    }
